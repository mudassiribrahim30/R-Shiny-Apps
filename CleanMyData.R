    ...
    sidebarPanel(
      ...
      div(
        ...
        fileInput(
          ...
          accept = c(".csv", ".sav", ".dta", ".xlsx", ".xls", ".zsav", ".por")
        ),
        radioGroupButtons(
          "header_option", 
          "Header Option",
...
server <- function(input, output, session) {
  ...
  # Function to clean and convert haven_labelled variables
  clean_haven_data <- function(df) {
    tryCatch({
      # Store original attributes for SPSS/Stata files
      original_attrs <- list()
      
      # Convert haven_labelled to simple types for DataTables compatibility
      for (col_name in names(df)) {
        # Store original attributes
        original_attrs[[col_name]] <- attributes(df[[col_name]])
        
        # Convert haven_labelled to appropriate type
        if (inherits(df[[col_name]], "haven_labelled")) {
          # Try to convert to numeric first, then factor if needed
          df[[col_name]] <- as.numeric(df[[col_name]])
        }
        
        # Also handle labelled vectors
        if (inherits(df[[col_name]], "labelled")) {
          df[[col_name]] <- as.numeric(df[[col_name]])
        }
      }
      ...
      # Store cleaned column names mapping
      attr(df, "cleaned_names") <- names(df)
      attr(df, "original_attrs") <- original_attrs
      
      return(df)
    }, error = function(e) {
      # Return the original df if cleaning fails
      return(df)
    })
  }
  ...
  safe_read_data <- function(path, ext, header_option, sep = ",", decimal = ".", header_row = NULL) {
    tryCatch({
      if (ext %in% c("sav", "dta", "zsav", "por")) {
        ...
        if (ext == "csv") {
          ...
          if (header_option == "first_row") {
            ...
            temp_df <- read_excel(path, col_names = FALSE)
            if (header_row <= nrow(temp_df)) {
              col_names <- as.character(temp_df[header_row, ])
              if (header_row < nrow(temp_df)) {
                df <- temp_df[-seq_len(header_row), ]
        ...
        # Clean column names for CSV/Excel files too
        names(df) <- gsub("[^[:alnum:]_\\.]", "_", names(df))
        names(df) <- gsub("_{2,}", "_", names(df))
        names(df) <- gsub("^_|_$", "", names(df))
        
        # Convert character to factors for categorical variables
        df <- df %>%
      ...
      # Convert all columns to appropriate types for DataTables
      df <- df %>%
  ...
  observeEvent(input$apply_dummy, {
    ...
    tryCatch({
      # Ensure the column is factor
      if (!is.factor(rv$data[[input$dummy_col]])) {
        rv$data[[input$dummy_col]] <- as.factor(rv$data[[input$dummy_col]])
      }
      
      # Create dummy variables using fastDummies
      dummy_data <- fastDummies::dummy_cols(rv$data[[input$dummy_col]], 
                                            remove_first_dummy = FALSE,
                                            remove_selected_columns = FALSE)
      
      # Get the dummy variable names (excluding the original column)
      dummy_names <- setdiff(names(dummy_data), input$dummy_col)
      
      # Add the dummy variables to the data
      for (dummy_name in dummy_names) {
        rv$data[[dummy_name]] <- dummy_data[[dummy_name]]
      }
  ...
  observeEvent(input$apply_header, {
    ...
    tryCatch({
      ...
      # Use the safe_read_data function
      rv$data <- safe_read_data(path, ext, input$header_option, 
                                input$sep, input$decimal, input$header_row)
  ...
  observeEvent(input$file, {
    ...
    tryCatch({
      ...
      rv$data <- safe_read_data(path, ext, input$header_option, 
                                input$sep, input$decimal, 
                                if(input$header_option == "specific_row") input$header_row else NULL)
  ...
  observeEvent(input$transpose, {
    ...
    tryCatch({
      # Convert to matrix first, then back to data frame
      transposed <- as.data.frame(t(as.matrix(rv$data)))
      # Clean column names
      names(transposed) <- paste0("V", seq_len(ncol(transposed)))
      transposed$RowNames <- rownames(transposed)
  ...
  observeEvent(input$apply_filter, {
    ...
    tryCatch({
      ...
      # Update last_saved version after filtering
      rv$last_saved <- rv$data
  ...
  # Display data table with editing capability - FIXED to handle haven_labelled variables
  output$data_table <- renderDT({
    req(rv$data)
    
    # Ensure all columns are simple types for DataTables
